...
export default function ForecastedAQI({ data }: ForecastedAQIProps) {
  ...
  // Filter data for selected location and next 5 hours
  const locationData = data
    .filter(
      (item) =>
        item.location === selectedLocation &&
        new Date(item.timestamp) > now &&
        new Date(item.timestamp) <= fiveHoursLater
    )
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-center text-muted-foreground">
        Forecasted values based on ML model prediction for next 5 hours
      </div>
    </div>
  )
}
